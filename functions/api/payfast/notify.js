/**
 * Cloudflare Pages Function — POST /api/payfast/notify
 *
 * PayFast Instant Transaction Notification (ITN) endpoint.
 * PayFast POSTs here after every payment event (COMPLETE, FAILED, etc.).
 *
 * Validation steps (per PayFast documentation):
 *  1. Reconstruct the signature and compare with the received one
 *  2. Confirm the payment_status is COMPLETE
 *  3. (Optionally) confirm the amount matches your order total
 *
 * Required env vars:
 *   PAYFAST_MERCHANT_ID
...
// Same MD5 implementation as create-payment.js
function md5(str) {
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const { signature: receivedSig, ...rest } = data;
    ...
    if (expectedSig !== receivedSig) {
      console.error(`PayFast ITN: signature mismatch — expected ${expectedSig}, got ${receivedSig}`);
    ...
    // ── 3. Log the result ──────────────────────────────────────────────────
    const { payment_status, m_payment_id, amount_gross, name_first, name_last, email_address } = data;
    ...
    console.log(JSON.stringify({
      ...
      payment_status,
      amount_gross,
      customer:       `${name_first} ${name_last} <${email_address}>`,
    }));

    // ── 4. Handle outcome ──────────────────────────────────────────────────
    if (payment_status === 'COMPLETE') {
      // TODO (when you add a database):
      //   - Mark order m_payment_id as paid
      //   - Trigger fulfilment / email confirmation
      console.log(`✅ Payment COMPLETE: ${m_payment_id} — R${amount_gross}`);
    } else {
      console.log(`⚠️  Payment ${payment_status}: ${m_payment_id}`);
    }

    // PayFast requires a 200 OK with empty body to acknowledge receipt
